 ...
 * Paralelismo para el cálculo de la función SENO calculando el tiempo de ejecución
 */
...
#include <sys/time.h> // Biblioteca para medir el tiempo de ejecución
#include <time.h> // Biblioteca para funciones de tiempo

#define MIN(x,y) (((x) < (y))? (x) :(y) ) // Macro para obtener el mínimo entre dos valores
struct timeval inicio, fin; // Estructuras para almacenar tiempos de inicio y fin
void InicioMuestra(){
    gettimeofday(&inicio, (void *)0); // Obtiene el tiempo actual y lo almacena en 'inicio'
}

void finMuestra(){
    gettimeofday(&fin, (void *)0); // Obtiene el tiempo actual y lo almacena en 'fin'
    fin.tv_usec -= inicio.tv_usec; // Calcula el tiempo total transcurrido
    fin.tv_sec  -= inicio.tv_sec;  // Calcula el tiempo total transcurrido
    double tiempo = (double) (fin.tv_sec * 1000000 + fin.tv_usec); // Convierte el tiempo a microsegundos
    printf("Tiempo de ejecución: %9.0f  \n", tiempo); // Muestra el tiempo de ejecución en microsegundos
}
...
int main(int argc, char *argv[]) { //se agrega argumento para numero de hilos
    ...
    finMuestra(); // Finaliza la medición del tiempo de ejecución
